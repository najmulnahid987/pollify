...
-- Policy: Users can read polls based on visibility
-- Public polls: accessible to all when published
...
-- Drop old policies if they exist
drop policy if exists "Poll creator can insert options" on poll_options;
...
-- Drop old policies if they exist
drop policy if exists "Poll creator can update options" on poll_options;

-- Policy: Poll creator can update options
create policy "Poll creator can update options" on poll_options
  for update using (
    (select user_id from polls wif not exists)
...
-- Drop old policies if they exist
drop policy if exists "Authenticated users can create responses" on poll_responsesCreate poll_responses table (for tracking votes/feedback)
...
-- Add visibility column to polls if it doesn't exist
alter table polls add column if not exists visibility varchar(10) default 'public' check (visibility in ('public', 'private'));
